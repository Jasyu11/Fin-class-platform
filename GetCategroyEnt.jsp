...
var count=0;
var obj;
var ch_category=decodeURI(getQueryVariable("ch_category"),"utf-8");
var display_count=10; //每页显示的内容数
...
window.onload=function(){
	...
	xmlhttp.onreadystatechange=function(){
		if(xmlhttp.readyState==4&&xmlhttp.status==200){
			...
			var str="<h1>"+ch_category+"</h1>";
			for(var i=0;i<current_page*display_count;i++){
				if(JSON.stringify(obj[i])){
				str+="<div class=ent_show>企业代号<font style="+"font-size:25px;"
		    	+">"+obj[i].name+"</font>";
		    	str+="<br><br>注册资本:<font style=font-size:30px>"+obj[i].registered_capital+"万元</font>"
		    	str+="企业分支数:<font style=font-size:30px>"+obj[i].number_of_branches+"个</font>"
		    	str+="企业职工数:<font style=font-size:30px>"+obj[i].number_of_employees+"人</font>";
		    	str+="企业网店数量:<font style=font-size:30px>"+obj[i].onlineshop_number+"个</font></div>"
				}
			}
			div_show.innerHTML=str;
			}
		}
		xmlhttp.open("GET","GetCategoryEnt?category="+category,"true");
		xmlhttp.send();
	}
function search(){
